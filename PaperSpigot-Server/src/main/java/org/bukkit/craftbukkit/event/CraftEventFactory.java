...
public class CraftEventFactory {
    ...
    public static PlayerDeathEvent callPlayerDeathEvent(EntityPlayer victim, List<org.bukkit.inventory.ItemStack> drops, String deathMessage, boolean keepInventory) {
        ...
        for (org.bukkit.inventory.ItemStack stack : event.getDrops()) {
            ...
            world.dropItemNaturally(entity.getLocation(), stack, (Entity) victim);
        }
	...
	public static PlayerPickupExperienceEvent callPlayerPickupExperienceEvent(EntityPlayer player, EntityExperienceOrb orb) {
		net.minecraft.server.World world = player.world;
		PlayerPickupExperienceEvent event = new PlayerPickupExperienceEvent(player.getBukkitEntity(), new CraftExperienceOrb(world.getServer(), orb));
		world.getServer().getPluginManager().callEvent(event);
		return event;
	}
