...
public class EntityItem extends Entity {
    ...
    public Entity owner; // NoksDevSpigot add
    ...
    public void h() {
        if (this.getItemStack() == null) {
            ...
            // NoksSpigot - Test start
            if ((this.inUnloadedChunk) && (this.world.paperSpigotConfig.removeUnloadedItems)) {
            	die();
            }
            // NoksSpigot - Test stop
    ...
    public boolean a(EntityItem entityitem) {
        if (entityitem == this) {
            ...
            if (itemstack1.getItem() != itemstack.getItem()) {
                ...
                return false;
            } else if(CraftEventFactory.callItemMergeEvent(entityitem, this).isCancelled()) {
            	return false;
            } else {
                // Spigot start
    ...
    public void b_(EntityHuman entityhuman) {
        if (!this.world.isStatic) {
        	// NoksioSpigot
        	if (!((EntityPlayer) entityhuman).getBukkitEntity().canSeeMore(getBukkitEntity())) {
        		return;
        	}
            ItemStack itemstack = this.getItemStack();
            ...
            boolean flyAtPlayer = false; // NoksioSpigot

            if (this.pickupDelay <= 0 && canHold > 0) {
                itemstack.count = canHold;
                PlayerPickupItemEvent event = new PlayerPickupItemEvent((org.bukkit.entity.Player) entityhuman.getBukkitEntity(), (org.bukkit.entity.Item) this.getBukkitEntity(), remaining);
                event.setCancelled(entityhuman.getBukkitEntity().getCanPickupItems());
                this.world.getServer().getPluginManager().callEvent(event);
                itemstack.count = canHold + remaining;
                flyAtPlayer = event.getFlyAtPlayer(); // NoksioSpigot
