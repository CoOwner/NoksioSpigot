...
public class LoginListener implements PacketLoginInListener {
    ...
    public void a() {
    	if (!MinecraftServer.getServer().isRunning()) {
    		this.disconnect(SpigotConfig.restartMessage);
    		return;
    	}
    	
        if (this.g == EnumProtocolState.READY_TO_ACCEPT) {
        	if(networkManager.isConnected()) { // NoksioSpigot adds
        		this.c();
        	}
        }
    ...
    public void disconnect(String s) {
        ...
        try {
            ...
            c.error("Error whilst disconnecting player {} for {}", name, s, exception);
        }
    }
    ...
    public void a(PacketLoginInStart packetlogininstart) {
        ...
        if (!this.networkManager.isProxied && this.server.getOnlineMode() && !this.networkManager.isLocal()) { // RinnySpigot - add !isProxied
