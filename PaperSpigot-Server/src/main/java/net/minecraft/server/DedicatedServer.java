...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
    ...
    //private final List<ServerCommand> serverCommandQueue = Collections.synchronizedList(new ArrayList<ServerCommand>());
    private final java.util.Queue<ServerCommand> serverCommandQueue = new java.util.concurrent.ConcurrentLinkedQueue<ServerCommand>(); // RinnySpigot
    ...
    protected boolean init() throws java.net.UnknownHostException { // CraftBukkit - throws UnknownHostException
        ...
        if (Runtime.getRuntime().maxMemory() / 1024L / 1024L <= 512L) {
            LOGGER.warn("To start the server with more ram, launch it as \"java -Xmx1024M -Xms1024M -jar "+new File(Main.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getName()+"\"");
        }
        ...
        if (eulaAgreed) {
            System.err.println( "You have used the RinnySpigot command line EULA agreement flag." );
            ...
            if (!EULALock.exists()) {
                ...
                System.err.println("The server will start in 3 seconds, you will only see this message and have to wait this one time.");
                try {
            ...
            // NoksioSpigot Start
            if (this.p != EnumGamemode.SURVIVAL) {
            ...
            // NoksioSpigot start
            NoksConfig.init();
            NoksConfig.registerCommands();
            // NoksioSpigot stop

            LOGGER.info("Generating keypair");
            this.a(MinecraftEncryption.b());
            LOGGER.info("Starting Minecraft server on " + (this.getServerIp().length() == 0 ? "*" : this.getServerIp()) + ":" + this.L());
        ...
        if (!org.spigotmc.SpigotConfig.lateBind) {
            ...
            if (!NameReferencingFileConverter.a(this.propertyManager)) {
                ...
                LOGGER.info("Preparing level \"" + this.O() + "\"");
