...
import us.noks.rinny.potion.PotionsConfig;
...
public class TileEntityBrewingStand extends TileEntity implements IWorldInventory {
    ...
    private Item m;
    private String n;
    public double brewSpeedMultiplier = 2.5; //Normal = 1.0 // PaperSpigot - brew speed multiplier API
    ...
    public void h() {
        ...
        if (this.brewTime > 0) {
            //this.brewTime -= (elapsedTicks * brewSpeedMultiplier);
        	this.brewTime -= !NoksConfig.fastBrew ? (elapsedTicks * 1.0) : (elapsedTicks * brewSpeedMultiplier);
            if (this.brewTime <= 0) { // == -> <=
                // CraftBukkit end
                this.l();
                this.update();
            ...
            this.brewTime = 400; //Peux être modif
