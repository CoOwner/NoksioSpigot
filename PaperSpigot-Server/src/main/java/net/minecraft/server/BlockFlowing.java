        ...
        if (this.q(world, i, j - 1, k)) {
            ...
            if (!event.isCancelled()) {
                if (this.material == Material.LAVA && world.getType(i, j - 1, k).getMaterial() == Material.WATER) {
                	BlockStoneFormEvent blockStoneFormEvent = new BlockStoneFormEvent(world.getWorld().getBlockAt(i, j - 1, k));
                	Bukkit.getServer().getPluginManager().callEvent(blockStoneFormEvent);
                	if (!blockStoneFormEvent.isCancelled()){
