        ...
        if (entity.av()) {
            if (!entity.j(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        if (i > 0) {
                        	entity.g((double) (-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * (float) i * NoksConfig.knockbackSprintForce), NoksConfig.knockbackSprintHeight, (double) (MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * (float) i * NoksConfig.knockbackSprintForce));
                        	long now = this.world.getTime();
                        	if (this.isSprinting() && this.getBukkitEntity().getLastSprintKnockback() + 20L > now) {
                        		this.motX *= 0.6D;
                            	this.motZ *= 0.6D;
                            	setSprinting(false);
                        	}
                        ...
                        if (entity instanceof EntityPlayer && entity.velocityChanged){
                        	...
                        	this.world.getServer().getPluginManager().callEvent(event);
                        	if (!event.isCancelled()){
                        		victim.getBukkitEntity().setVelocityDirect(event.getVelocity());
