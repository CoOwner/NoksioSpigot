...
public abstract class Entity {
    ...
    public static Random SHARED_RANDOM = new Random(); // RinnySpigot
    ...
    public void setLocation(double d0, double d1, double d2, float f, float f1) {
        ...
        world.getChunkAt((int) Math.floor(this.locX) >> 4, (int) Math.floor(this.locZ) >> 4); // RinnySpigot - Ensure chunks are always loaded on hard positions set.
    ...
    public void ab() {
        if (this.vehicle.dead) {
        	if ((this instanceof EntityLiving)) {
        		((EntityLiving)this).m(this.vehicle);
        	}
            this.vehicle = null;
        } else {
            this.motX = 0.0D;
