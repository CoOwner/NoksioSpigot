...
public abstract class EntityLiving extends Entity {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                */
                ...
                // PaperSpigot end
                	...
                	if (entity != null) {
                		double xo = entity.locX - this.locX;
                		double zo;
                		
                		for (zo = entity.locZ - this.locZ; xo * xo + zo * zo < 1.0E-4D; zo = (Math.random() - Math.random()) * 0.01D) {
                			xo = (Math.random() - Math.random()) * 0.01D;
                		}
                		this.az = (float) (Math.atan2(zo, xo) * 180.0D / 3.1415927410125732D) - this.yaw;
                		this.a(entity, f, xo, zo);
                	} else {
	...
	public void a(Entity attacker, float f, double xo, double zo) {
		if (this.random.nextDouble() >= getAttributeInstance(GenericAttributes.c).getValue()) {
			this.impulse = true;
			double magnitude = MathHelper.sqrt(xo * xo + zo * zo);
        
			this.motX /= NoksConfig.knockbackFriction;
			this.motY /= NoksConfig.knockbackFriction;
			this.motZ /= NoksConfig.knockbackFriction;
			
			this.motX -= xo / magnitude * NoksConfig.knockbackForce;
			this.motY += NoksConfig.knockbackHeight;
			this.motZ -= zo / magnitude * NoksConfig.knockbackForce;
			if (this.motY > NoksConfig.knockbackHeightLimit) { //if (this.motY > 0.4000000059604645D) {
				this.motY = NoksConfig.knockbackHeightLimit;
			}
		}
	}
    ...
    protected void bo() {
    	if (this.R()) { // Spigot: Add this.R() condition
        ...
        if (this.R() && list != null && !list.isEmpty()) { // Spigot: Add this.R() condition
        	...
        	numCollisions = Math.max(0, numCollisions - world.spigotConfig.maxCollisionsPerEntity); // NoksioSpigot Fix
    ...
    public boolean hasLineOfSight(Entity entity) {
        return this.world.rayTrace(Vec3D.a(this.locX, this.locY + (double) this.getHeadHeight(), this.locZ), Vec3D.a(entity.locX, entity.locY + (double) entity.getHeadHeight(), entity.locZ)) == null;
    }
