...
public class EntityPotion extends EntityProjectile {
    ...
    protected float i() { //VITESSE
    	return 0.05F;
    }

    protected float e() { //DISTANCE
    	return 0.5F;
    }

    protected float f() { //VERTICALE
    	return -20.0F;
    }
        ...
        if (!this.world.isStatic) {
            ...
            if (true || list != null && !list.isEmpty()) { // CraftBukkit - Call event even if no effects to apply
                ...
                if (list1 != null) { // CraftBukkit - Run code even if there are no entities around
                    ...
                    // CraftBukkit
                    Map<LivingEntity, Double> affected = new HashMap<LivingEntity, Double>(); // RinnySpigot - HashMap -> Map
                    ...
                    if (!event.isCancelled() && list != null && !list.isEmpty()) { // do not process effects if there are no effects to process
                        for (LivingEntity victim : event.getAffectedEntities()) {
                            ...
                            while (iterator1.hasNext()) {
                            	...
                            	// CraftBukkit end
                            	if (entityliving instanceof EntityPlayer && !((EntityPlayer) entityliving).getBukkitEntity().canSeeMore(this.getBukkitEntity())) {
                                    continue;
                                }
