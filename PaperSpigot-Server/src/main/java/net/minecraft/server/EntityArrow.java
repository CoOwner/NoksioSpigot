...
public class EntityArrow extends Entity implements IProjectile {
    ...
    public void inactiveTick() {
        if (this.inGround) {
        	//OLD 19
            this.at += 1; // Despawn counter. First int after shooter
        }
        super.inactiveTick();
    }
    // Spigot End
    ...
    public void h() {
        ...
        if (this.inGround) {
            ...
            if (this.isCritical()) {
                for (int x = 0; x < 4; x++) {
                    this.world.addParticle("crit", this.locX + this.motX * (double) x / 4.0D, this.locY + this.motY * (double) x / 4.0D, this.locZ + this.motZ * (double) x / 4.0D, -this.motX, -this.motY + 0.2D, -this.motZ);
                }
            }
