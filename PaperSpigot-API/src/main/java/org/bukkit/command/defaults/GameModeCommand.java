    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (player != null) {
            ...
            if (mode != player.getGameMode()) {
                ...
                if (mode != player.getGameMode()) {
                    sender.sendMessage("§cGamemode change for " + player.getName() + " error!");
                } else {
                    if (player == sender) {
                        sender.sendMessage(ChatColor.GOLD + "You are now in " + ChatColor.YELLOW + mode.toString() + ChatColor.GOLD + " mode");
                    } else {
                        sender.sendMessage(ChatColor.GOLD + "You have set " + ChatColor.RED + player.getName() + ChatColor.GOLD + " in " + ChatColor.YELLOW + mode.toString() + ChatColor.GOLD + " mode");
                        player.sendMessage(ChatColor.GOLD + "You are now in " + ChatColor.YELLOW + mode.toString() + ChatColor.GOLD + " mode");
                    }
                }
            } else {
                sender.sendMessage("§c" + player.getName() + " is already in " + mode.toString() + " mode!");
            }
        } else {
            sender.sendMessage("§cUnknown " + playerArg + " player!");
        }
